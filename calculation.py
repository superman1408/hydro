...
def sea_environment_calculation(frontend):
    try:
        ...
        input_data = {
            ...
            "water_depth":frontend["water_depth"],
            "wave_propagation":frontend["wave_propagation"],
            "coordinates":frontend["coordinates"],
            "vertical_coordinate":frontend["vertical_coordinate"],
            "time":frontend["time"]
        ...
        water_depth = input_data(water_depth)
        wave_propagation = input_data(wave_propagation)
        coordinates = input_data(coordinates)
        vertical_coordinate = input_data(vertical_coordinate)
...
# #     return circular_Frequency_omega / math.sqrt(constant["g"] * average_Water_Depth_h)
...
# # Evaluate the constant left side target: constant = ω² / g
# target_value = (circular_frequency ** 2) / constant["g"]
# constant_line = [target_value for _ in k_values]
